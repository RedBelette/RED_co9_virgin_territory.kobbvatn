...
blz_repair =
{
	...
	if(_damage > 0 && triggerActivated _trg) then
	{
		...
		{
			titleText ["Reparation en cours...","PLAIN DOWN"];
			sleep sleeptimer_s;
	...
	if(_fuel < 1 && triggerActivated _trg) then
		{
			...
			while {_fuel < 1 && triggerActivated _trg} do
				{
					titleText ["Plein d'essence en cours...","PLAIN DOWN"];
					sleep sleeptimer_s;
	...
	if(count _weapon > 1 && triggerActivated _trg) then
		{
				...
				{
					titleText ["Réarmement en cours...","PLAIN DOWN"];
					sleep sleeptimer_m;
...
/*______________________________________________________________________________________________*/
	...
	if (_count_turrets > 0) Then {
		...
		{
			titleText ["Réarmement de la tourrelle...","PLAIN DOWN"];
			sleep sleeptimer_m;
	...
	if (_count_turrets_cmd > 0) Then {
		...
		{
			titleText ["Réarmement du poste de commandeur...","PLAIN DOWN"];
			sleep sleeptimer_m;
...
if (triggerActivated _trg) then {
	titleText ["Mise en place du véhicule...", "PLAIN"];
	sleep sleeptimer_s;
	if (triggerActivated _trg) then {
...
into =
{
	...
	sleep sleeptimer_s;
	titleText ["Voyons voir...", "PLAIN DOWN"];
...
{
	sleep sleeptimer_s;
	titleText ["Ok, on est bon!", "PLAIN DOWN"],
...
if (triggerActivated _trg && _object != player && player == driver _object) then {
	sleep sleeptimer_s;
			...
			if(triggerActivated _trg) Then {
				SS = driver _object addAction ["<t color='#f69619'>Station Service</t>", {call blz_ss}, [_object,_trg,_trg_pos,_trg_agl,_trg_txt]];

				if(_trg_txt == "blz_turn") Then
					{
						SS_turn = driver _object addAction ["<t color='#f69619'>Station Service avec mise en place du véhicule.</t>", {call blz_ss_turn}, [_object,_trg,_trg_pos,_trg_agl,_trg_txt]];
					};
